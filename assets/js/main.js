...
const isOnline = () => {
	if (navigator.onLine) {
		toastMessage('success', 'Conectado a internet').showToast();
	} else {
		toastMessage('success', 'Estas offline').showToast();
	}
};
...
const verifyNotifications = (activated) => {
	if (activated) {
		...
		$('#notifyDesactivated').css('display', 'none');
	} else {
		$('#notifyActivated').css('display', 'none');
		$('#notifyDesactivated').css('display', 'block');
	}
};
